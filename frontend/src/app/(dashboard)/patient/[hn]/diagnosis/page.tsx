...
const ENDPOINTS = {
  ...
  // ✅ ใช้แบบ ?patients_id= ให้ตรงกับ backend ที่คุณทดสอบว่าใช้ได้
  listDx: (id: string) => `${API_BASE}/api/patient_diagnosis?patients_id=${encodeURIComponent(id)}`,
  createDx: () => `${API_BASE}/api/patient_diagnosis`,
  updateDx: (diag_id: number) => `${API_BASE}/api/patient_diagnosis/${diag_id}`,
  deleteDx: (diag_id: number) => `${API_BASE}/api/patient_diagnosis/${diag_id}`,
};
...
export default function PatientDiagnosisPage() {
    ...
    async function submitForm(e: React.FormEvent) {
        ...
        try {
            ...
            if (msg.includes('unique') || msg.includes('23505')) {
                setMessage('ตั้ง Primary ซ้ำ: มี Primary ที่ active อยู่แล้ว')
            } else {
                setMessage(msg)
            }
        }
    ...
    async function onDelete(diag_id: number) {
        try {
    ...
    const goToPatient = () => {
        ...
        const url = `/patient/${encodeURIComponent(hnNext)}/diagnosis` // ← ใช้ /patient ไม่ใช่ /patients
        if (hnNext === (hn || '')) {
            loadAll()
        } else {
            router.push(url)
